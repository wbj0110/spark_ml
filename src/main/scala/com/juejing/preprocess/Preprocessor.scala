...
trait Preprocessor {
  ...
  /**
    ...
    * @return (预处理后的数据, 索引模型, 向量模型), 数据包括字段: "label", "indexedLabel", ... "features"
    */
  def forPredict(filePath: String, spark: SparkSession): (DataFrame, StringIndexerModel, CountVectorizerModel)

}

object Preprocessor {
  def apply( conf: Conf,preprocessor: String="china_news"): Preprocessor = {
    preprocessor match {
      case "china_news" => new ChinaNewsPreprocessor(conf)
      case _ => null
    }
  }
}